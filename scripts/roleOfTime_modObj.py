...
for budget in searchBudget:
    ...
    for run in range(nRestarts):
        ...
        expRR      = []             # List to collect expected rewards(t)
        ...
        for ii in range(n_trials):
            ...
            grad_mean = np.mean(grads, axis=0) / (a1_*budget + a2_) # mean gradient of exp reward rate
            ...
            if np.mod(ii,10000)==0:
                ...
                expRR = np.mean(rewards)/(a1_*budget + a2_)
                ...
                print(f"Objective = {np.around(objective,1)}, "+\
                    f"Reward Rate = {np.around(expRR,1)}, "+\
...
f = open(path+f"mod_df_a1_{int(a1_*10)}", 'wb')
pickle.dump(df_allRuns, f, protocol=pickle.HIGHEST_PROTOCOL)
f.close()

np.save(path + f"mod_R_a1_{int(a1_*10)}", expR_allRuns)
np.save(path + f"mod_C_a1_{int(a1_*10)}", cost_allRuns)
np.save(path + f"mod_O_a1_{int(a1_*10)}", obj_allRuns)